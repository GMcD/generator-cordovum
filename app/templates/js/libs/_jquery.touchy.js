 ...
 */
 ...
 define(['jquery'], function($){
    ...
    /*
     * To be implemented
     * the user touches the element before further interaction.  maybe abstracts touchstart and mousedown?
     *
     press: { 
     requiredTouches: 1,
     data: {},
    ...
    /*
     * To be implemented
     * the user touches the element at any time during the interaction.  could fire mouseenter as well as its own event, if configured to do so?
     *
     contact: {
     requiredTouches: 1,
    ...
    /*
     * To be implemented
     * the user taps twice within a short period of time on the element as the only interaction. 
     *
     dbltap: { 
     requiredTouches: 1,
     msThreshTap: 200,
...
var proxyHandlers = {
        ...
        if ($target) {
            ...
            if (touches.length === settings.requiredTouches) {
                switch (eventType) {
                    ...
                    data.startDate = e.timeStamp;
                    if (settings.triggerStartPhase) {
                        $target.trigger('touchy-tap', ['start', $target]);
                    }
                    data.timer = setTimeout($.proxy(function(){
                        $target.trigger('touchy-tap', ['end', $target]);
                    }, this), settings.msThresh);
                    break;
                    ...
                    if (touches.length === 1) {
                        ...
                        var rpoints = getTwoTouchPointData(e);
                        data.startPoint = {
                            "x": rpoints.centerX,
                            "y": rpoints.centerY
                        };
    ...
    handleGestureChange: function (e) {
        ...
        if ($target) {
            ...
            if (data.preventDefault && data.preventDefault.move) {
                event.preventDefault();
            }
    ...
    hasGestureChange = function () {
        return (window.ongesturechange !== null) && (typeof window.ongesturechange === "object");
    },
        ...
        function addOffset(node, coords, view) {
            ...
            if (p) {
                ...
                if (node.localName === 'BODY') {
                    var bstyle = view.getComputedStyle(node, '');
                    coords.x += parseInt(bstyle.borderLeftWidth, 10);
                    coords.y += parseInt(bstyle.borderTopWidth, 10);
